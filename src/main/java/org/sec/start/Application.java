...
import org.sec.utils.ClassUtil;
...
class CommandChoiceTest extends CommandChoice {
    ...
    public static void Analysis(Command command, boolean delete) throws IOException, ClassNotFoundException {
        if (command.dict != null) {
            ...
            FileUtils.readDir(command.dict,fileArray);
            ...
            for (String webDir : fileArray) {
                ...
                FileUtils.flushDir("temp");
                ArrayList<String> jarFilePathList = new ArrayList<>();
                FileUtils.getWantSuffixFilePath(webDir, "jar", jarFilePathList);
                ...
                // 编译 jar 包为对应的 class 文件
                ClassUtil.getAllClassesFromJars(jarFilePathList, false);

                ArrayList<String> classFileNameList = new ArrayList<>();
                FileUtils.getWantSuffixFilePath("temp", "class", classFileNameList);
                ...
                for (String classFileName : classFileNameList) {
                    ...
                    // 形成class文件和被扫描的jar之间的对应
                    String rootPath = new File("Compile").getAbsolutePath() + File.separator + "org" + File.separator + "apache" + File.separator + "jsp";
                ...
                if (!Constant.compileErrorFileNameList.isEmpty()) {
                    System.out.println("[-] 编译失败的文件:");
                    System.out.println(Constant.compileErrorFileNameList);
                    System.out.println("\n---------------------------------------------------------------------------\n" + "[+] 扫描结束");
                }

                //删除编译文件
                FileUtils.delete(new File("Compile"));
            }
        } else {
